...
void keyboardListener(Warehouse& warehouse) {
    ...
    while (getline(cin, line)) {
        ...
        if (command == "GEN") {
            ...
            bool success = warehouse.processCommand(line);
            if (!success) {
...
int main(int argc, char *argv[]) {
    ...
    bool saw_T = false, saw_U = false, saw_F = false;
    int option_index = 0;
    int c;
    while ((c = getopt_long(argc, argv, "T:U:f:s:d:t:", long_options, &option_index)) != -1) {
        switch (c) {
            case 'T':
    ...
    if (!saw_T || !saw_U || !saw_F) {
        cerr << "Error: -T, -U, and -f <filepath> are required.\n";
        print_usage(argv[0]);
    }
    if (optind < argc) {
    ...
    Warehouse warehouse;
    warehouse.setStoragePath(storage_file);
    if (!warehouse.loadFromFile()) {
        // If loading failed (e.g., file didn't exist), save initial state
    ...
    cout << ", timeout " << timeout_secs << " seconds." << endl;
    ...
    while (true) {
        ...
        if (activity == 0) {
            cout << "No requests for " << timeout_secs << " seconds. Shutting down." << endl;
            break;
        }
        ...
        for (int i = 0; i <= max_fd; ++i) {
            ...
            // UDP
                ...
                if (n > 0) {
                    ...
                    sendto(udp_fd, response.c_str(), response.size(), 0,
                           (sockaddr*)&client_addr, len);
                    cout << "Responded to UDP client." << endl;
                }
            }
            // UDS stream: new connection
                ...
                cout << "UDS stream client connected." << endl;
            }
            // UDS datagram
                ...
                if (n > 0) {
                    ...
                    sendto(uds_datagram_fd, response.c_str(), response.size(), 0,
                           (sockaddr*)&client_un, len);
                    cout << "Responded to UDS datagram client." << endl;
                }
            }
